    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          "http://localhost:5000/api/admin/outgoingComplaints?type=admin&user_id=1"
        );
    ...
    <Fragment>
      ...
      <div className={styles.conteant_page}>
        ...
        <div className="bg-light">
          <h3 className="text-center mt-5">الشكاوي الصادرة </h3>
          ...
          <div className="table-responsive m-2">
            <Table striped bordered hover>
              ...
              <tbody className="text-center align-middle">
                {assignments.filter((comp )=> comp.status !== "return" && employees.some((emp) => emp.id === comp.to_user_id)) 
                .map((item, idx) => (
                  <tr key={idx}>
                    ...
                    <td>{getEmployeeName(item.to_user_id, employees)}</td>
                ...
                {assignments.length === 0 && (
                  <tr>
                    <td colSpan={8}> لا يوجد شكاوي صادرة</td>
                  </tr>
                )}
...
export default ComplaintsIssuedAdmin;
